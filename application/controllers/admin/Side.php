...
Class Side extends My_Controller {
	...
	function index(){
		...
		$this->data['temp'] = 'admin/side/index';
		...
		if($this->input->post()){
			if($this->input->post('name_menu1') == '' || $this->input->post('lienket') == ''){
				...
				if($this->form_validation->run()){
					...
					if($module_menu == 2){
						$input['where'] = array('id' => $name_menu);
	...
	function edit(){
		...
		$id = intval($id);
		$info = $this->side_model->get_info($id);
		if(!$info)
		{
		...
		if($this->input->post())
		{
			...
			if($this->side_model->update($id, $data))
			{
			...
			redirect(admin_url('side/edit/'.$id));
		}

		$this->data['temp'] = 'admin/side/edit';
		$this->load->view('admin/main', $this->data);
		$this->db->cache_delete_all();
	}
	...
	private function _del($id, $redirect = true){
		...
		if(count($this->side_model->menucon($info->id))>0){
