...
class MY_Model extends CI_Model {
    ...
    // de quy danh mục menu
	function dequydm($parent_id = 0, $data = NULL){
	    if(isset($data) && $data){
	        foreach ($data as $key => $val){
	            if($val->parent_id == $parent_id){
	                $this->recursive[] = $val->id;
	                $this->dequydm($val->id, $data);
	            }
	        }
	    }
    ...
    // de quy danh mục con
	function dequydmcon($doituong = array()){

		if($doituong->parent_id == 0){
			pre($doituong);
		}
	    // if(isset($data) && $data){
	    //     foreach ($data as $key => $val){
