	...
	<form class="form" id="form" action=" " method="post" enctype="multipart/form-data">
		<fieldset>
			...
			<?php $this->load->view('admin/message', $this->data); ?>
				...
				<div class="formRight">
					<span class="oneTwo"><input name="username" id="param_name" _autocheck="true" value="<?php echo $info->username; ?>" type="text" /></span>
				...
				<div class="formRight">
					...
					<p style="color: red">Nếu cập nhật mật khẩu thì mới nhập giá trị</p>
				</div>
				<div class="clear"></div>
				</div>
				<div class="formRow hide"></div>	
					...
					<div class="formRight">
						<table>
							<?php foreach($config_quyen as $controller => $actions):?>
								...
								<?php 
								$item_quyen = array();
								if(isset($info->quyen->{$controller})){
									$item_quyen = $info->quyen->{$controller};
										}
								?>
							...
							<tr>
								...
								<td style="padding: 0px 10px 10px 0px"><input type="checkbox" name="quyen[<?= $controller?>][]" value="<?= $item?>" <?= in_array($item, $item_quyen) ? 'checked' : '' ?> ><?= $item ?></td>
							<?php endforeach; ?>
							</tr>
						<?php endforeach;?>
						</table>
				...
				<div class="formRight">
					<span class="oneTwo"><input name="tongtien" id="tongtien"  value="<?php echo $info->tongtien; ?>" type="number" /></span>
				</div>
				<div class="clear"></div>
				</div>
