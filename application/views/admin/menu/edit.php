	...
	<form class="form" id="form" action=" " method="post" enctype="multipart/form-data">
		<fieldset>
			...
			<?php $this->load->view('admin/message', $this->data); ?>
				...
				<div class="formRow" <?=($infosetting->ngonngu == 0)?'style="display:none"':''?>>
				...
				<div class="formRow">
					...
					<div class="formRight">
			            ...
			            <option value="<?= $row->id ?>" <?= ($info->parent_id == $row->id)? 'selected' : ''?>><?=$row->name?> <?=($infosetting->ngonngu != 0)?'('.$row->name_en.')':''?></option>
						<?php if(count($this->menu_model->menucon($row->id)) > 0):?>
			            	<?php foreach ($this->menu_model->menucon($row->id) as $con): ?>
								<option value="<?= $con->id ?>" <?= ($info->parent_id == $con->id)? 'selected' : ''?>><?= '--|'.$con->name?> <?=($infosetting->ngonngu != 0)?'('.$con->name_en.')':''?></option>
